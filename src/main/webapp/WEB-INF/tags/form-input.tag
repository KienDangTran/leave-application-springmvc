    ...
    <div class="form-group ${empty colClass ? 'col-md-6' : colClass} ${status.error ? 'has-error' : ''}">
        ...
        <c:choose>
            <c:when test="${type eq 'date'}">
                <i>(yyyy-mm-dd)</i>
                <form:input type="text" cssClass="form-control datepicker ${inputClass}" id="${id}"
                            placeholder="${placeholder}" path="${path}" readonly="${readonly}" required="${required}"/>
            </c:when>
            ...
            <c:otherwise>
                <form:input type="${empty type ? 'text' : type}"
                            cssClass="form-control ${inputClass} ${identity ? 'identity' : ''}" id="${id}"
